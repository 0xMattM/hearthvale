/**
 * Postgres Drizzle schema (F16.1) — column shapes match SQLite for shared query code.
 */
...
export const craftJobs = pgTable("craft_jobs", {
  id: text("id").primaryKey(),
  playerId: text("player_id")
    .notNull()
    .references(() => players.id),
...
export const claimNodes = pgTable("claim_nodes", {
  id: text("id").primaryKey(),
...
export const claimContestScores = pgTable("claim_contest_scores", {
  id: text("id").primaryKey(),
...
export const landDeeds = pgTable("land_deeds", {
  id: text("id").primaryKey(),
  playerId: text("player_id")
    .notNull()
    .references(() => players.id),
...
export const coinSwaps = pgTable("coin_swaps", {
  id: text("id").primaryKey(),
  playerId: text("player_id")
    .notNull()
    .references(() => players.id),
...
export const tokenItemListings = pgTable("token_item_listings", {
  id: text("id").primaryKey(),
  sellerPlayerId: text("seller_player_id")
    .notNull()
    .references(() => players.id),
...
export const realmBalances = pgTable("realm_balances", {
  playerId: text("player_id")
    .primaryKey()
    .references(() => players.id),
...
export const localLands = pgTable("local_lands", {
  id: text("id").primaryKey(),
  playerId: text("player_id")
    .notNull()
    .references(() => players.id),
