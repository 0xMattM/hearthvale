...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl892-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL89.2 — Mail cancel returns escrow still green.
 * Choice: assert-only send → cancel restore (parity with CL76.1; no mail invent).
 */
describe("CityLands CL89.2 Mail cancel returns escrow still green", () => {
  let senderId = "";
  let receiverId = "";
  let otherId = "";
  let senderPlayerId = "";
  ...
  beforeAll(() => {
    ...
    const a = registerUser(`cl892a_${stamp}`, "password123");
    const b = registerUser(`cl892b_${stamp}`, "password123");
    const c = registerUser(`cl892c_${stamp}`, "password123");
  ...
  it("keeps cancelled parcel out of recipient inbox claim path (edge)", () => {
    ...
    expect(inbox.every((m) => m.subject !== "CL89.2 cancel me")).toBe(true);
  });
