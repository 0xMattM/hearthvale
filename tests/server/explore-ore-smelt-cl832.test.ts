...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl832-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL83.2 — Explore ore → land smelt still green.
 * Choice: assert-only fidelity (parity with CL71.2 / CL56.1; no Content Lock retune).
 */
describe("CityLands CL83.2 Explore ore → land smelt_iron_bar still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl832_${Date.now().toString(36)}`,
      "password123",
    );
