...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl841-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL84.1 — land wood_crate → City vendor sell still green; empty refuse.
 * Choice: assert-only fidelity (parity with CL72.3 / CL39.1; no Content Lock retune).
 */
describe("CityLands CL84.1 land wood_crate City vendor sink still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl841_${Date.now().toString(36)}`,
      "password123",
    );
