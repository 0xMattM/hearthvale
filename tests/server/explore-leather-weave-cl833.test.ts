...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl833-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL83.3 — Explore leather → land weave still green.
 * Choice: assert-only fidelity (parity with CL68.2; no Content Lock retune).
 */
describe("CityLands CL83.3 Explore leather → land loom weave still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl833_${Date.now().toString(36)}`,
      "password123",
    );
