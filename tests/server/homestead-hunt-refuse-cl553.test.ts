...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl553-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL55.3 — homestead hunt refuse still green; Explore trail/thicket OK.
 * Choice: assert-only huntExploreOnly hold over inventing land hunt nodes.
 */
describe("CityLands CL55.3 homestead hunt refuse still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl553_${Date.now().toString(36)}`,
      "password123",
    );
