...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl371-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/** Premium hunt mat — Explore book pays more than City (CL10.2 / CL37.1). */
...
describe("CityLands CL37.1 Explore vendor premium sell smoke", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(`cl371_${Date.now().toString(36)}`, "password123");
  ...
  it("pays Explore premium over City rate on live sell (happy)", () => {
