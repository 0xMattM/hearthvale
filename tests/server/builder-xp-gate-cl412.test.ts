...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl412-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
describe("CityLands CL41.2 builder XP gate on fishing_dock", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl412_${Date.now().toString(36)}`,
      "password123",
    );
  ...
  it("places fishing_dock after enough builder XP (happy)", () => {
    expect(PLAYER_LAND_STATIONS.fishing_dock.minBuilderXp).toBe(
      BUILDER_PLACE_XP,
    );
    ...
    expect(
      getPlayerState(userId)!.buildings.some((b) => b.type === "fishing_dock"),
    ).toBe(true);
    expect(getPlayerState(userId)!.builderXp).toBe(BUILDER_PLACE_XP * 2);
  });

  it("keeps crop_plot and workshop ungated at 0 builder XP (edge)", () => {
    const home = getPlayerState(userId)!;
    db.update(players)
  ...
  it("refuses fishing_dock under builder XP gate (failure)", () => {
    const home = getPlayerState(userId)!;
    db.update(players)
      .set({ softCurrency: 200, energy: 100, builderXp: 0 })
      .where(eq(players.id, home.playerId))
    ...
    const result = placeLandStation(userId, "fishing_dock", boardPos(home));
