...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl691-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL69.1 — Mail / offline parcel claim still green.
 * Choice: assert-only send → claim escrow (no mail system invent).
 */
describe("CityLands CL69.1 Mail / offline parcel claim still green", () => {
  let senderId = "";
  let receiverId = "";
  let senderPlayerId = "";
  let receiverName = "";
  ...
  beforeAll(() => {
    ...
    const a = registerUser(`cl691a_${stamp}`, "password123");
    const b = registerUser(`cl691b_${stamp}`, "password123");
  ...
  it("rejects claim of missing parcel (failure)", () => {
    const missing = claimMail(receiverId, "mail_missing_cl691");
