...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl792-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL79.2 — City scarce tree/ore gather contention still green.
 * Choice: assert-only soft presence lock (parity with CL60.1 / CL60.2; no qty caps).
 */
describe("CityLands CL79.2 city scarce tree/ore gather contention still green", () => {
  let aliceId = "";
  let aliceName = "";
  let bobId = "";
  let bobName = "";
  ...
  beforeAll(() => {
    ...
    aliceName = `cl792a_${stamp}`;
    bobName = `cl792b_${stamp}`;
