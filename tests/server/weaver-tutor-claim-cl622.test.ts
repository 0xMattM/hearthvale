...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl622-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL62.2 — Weaver tutor claim after land weave still green.
 * Choice: land loom `weave_cloth` → City claim (assert-only objective weave_cloth).
 */
describe("CityLands CL62.2 Weaver tutor claim after land weave", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl622_${Date.now().toString(36)}`,
      "password123",
    );
  ...
  it("keeps carpenter inactive after weave-only (edge)", () => {
    // Reason: loom place leaves plank leftovers — clear so weave-only ≠ carpenter ready.
