...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl473-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/** Content Lock / CL47.3 — bread NPC buyback (city / explore / land). */
const BREAD_SELL = 3;
...
describe("CityLands CL47.3 land bread → City vendor sell e2e", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl473_${Date.now().toString(36)}`,
      "password123",
    );
  ...
  it("bakes bread on land then sells at City vendor (happy)", () => {
    const recipe = getRecipe("bake_bread")!;
    ...
    expect(getVendorPrices("player_land").sell.bread).toBe(BREAD_SELL);
    expect(getVendorPrices("explore").sell.bread).toBe(BREAD_SELL);
    ...
    db.update(players)
      .set({
        softCurrency: 200,
        energy: 100,
        // Reason: mill gated; kitchen stays cook bootstrap.
    ...
    addItem(home.playerId, "flour", 2);

    // Reason: e2e sells the land-baked loaf — clear starter bread first.
