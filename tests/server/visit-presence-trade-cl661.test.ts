...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl661-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL66.1 — Visit presence + nearby trade still green.
 * Choice: assert-only visit host presence + trade ping (CL49.1 / CL52.2) over new social systems.
 */
describe("CityLands CL66.1 visit presence + nearby trade still green", () => {
  let aliceId = "";
  let aliceName = "";
  let bobId = "";
  let bobName = "";
  ...
  beforeAll(() => {
    ...
    aliceName = `cl661a_${stamp}`;
    bobName = `cl661b_${stamp}`;
