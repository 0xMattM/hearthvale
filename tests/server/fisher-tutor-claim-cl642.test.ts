...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl642-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL64.2 — Fisher tutor claim after land dock catch still green.
 * Choice: land fishing_dock catch → City claim (assert-only objective hold_fish).
 */
describe("CityLands CL64.2 Fisher tutor claim after land dock catch", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl642_${Date.now().toString(36)}`,
      "password123",
    );
