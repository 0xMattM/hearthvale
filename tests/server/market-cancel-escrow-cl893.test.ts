...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl893-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL89.3 — Market cancel returns escrow still green.
 * Choice: assert-only list → cancel restore (parity with CL81.3; no market invent).
 */
describe("CityLands CL89.3 Market cancel returns escrow still green", () => {
  let sellerId = "";
  let otherId = "";
  ...
  beforeAll(() => {
    ...
    const s = registerUser(`cl893s_${stamp}`, "password123");
    const o = registerUser(`cl893o_${stamp}`, "password123");
