...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl511-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/** Content Lock / CL51.1 — cooked fish NPC buyback (city / explore / land). */
...
describe("CityLands CL51.1 land cook_fish → City vendor sell e2e", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl511_${Date.now().toString(36)}`,
      "password123",
    );
