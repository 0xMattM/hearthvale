...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl813-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL81.3 — Market cancel returns escrow still green.
 * Choice: assert-only list → cancel restore (parity with CL69.3; no market invent).
 */
describe("CityLands CL81.3 Market cancel returns escrow still green", () => {
  let sellerId = "";
  let otherId = "";
  ...
  beforeAll(() => {
    ...
    const s = registerUser(`cl813s_${stamp}`, "password123");
    const o = registerUser(`cl813o_${stamp}`, "password123");
