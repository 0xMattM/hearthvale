...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl873-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL87.3 — Land unlimited craft (no stationBusy) still green.
 * Choice: assert-only peer on player_land does not block (parity with CL79.3 / CL52.3 land edge; no invent caps).
 */
describe("CityLands CL87.3 land unlimited craft (no stationBusy) still green", () => {
  let aliceId = "";
  let aliceName = "";
  let bobId = "";
  let bobName = "";
  ...
  beforeAll(() => {
    ...
    aliceName = `cl873a_${stamp}`;
    bobName = `cl873b_${stamp}`;
