...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl571-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
describe("CityLands CL57.1 city scarce forge contention assert", () => {
  let aliceId = "";
  let aliceName = "";
  let bobId = "";
  let bobName = "";
  ...
  beforeAll(() => {
    ...
    aliceName = `cl571a_${stamp}`;
    bobName = `cl571b_${stamp}`;
  ...
  it("peer presence does not block city craft start (wait/collect override)", () => {
    ...
    addItem(bob.id, "iron_ore", 2);
    const recipe = getRecipe("smelt_iron_bar")!;
    expect(recipe.station).toBe("forge");

    const blocked = craftRecipe(bobId, "smelt_iron_bar", pos);
    expect(blocked.ok).toBe(true);
  });

  it("alone at city forge crafts OK; land unlimited ignores peer presence (edge)", () => {
    const city = getPlayerState(aliceId)!;
    const forge = city.buildings.find((b) => b.type === "forge")!;
    const pos = buildingPos(forge);
    ...
    expect(craftRecipe(aliceId, "smelt_iron_bar", pos).ok).toBe(true);
    ...
    expect(placeLandStation(aliceId, "forge", atBoard).ok).toBe(true);
    const landForge = getPlayerState(aliceId)!.buildings.find(
      (b) => b.type === "forge",
    )!;
    const landPos = buildingPos(landForge);
    addItem(getPlayerState(aliceId)!.playerId, "iron_ore", 2);
    ...
    expect(craftRecipe(aliceId, "smelt_iron_bar", landPos).ok).toBe(true);
  });
});
