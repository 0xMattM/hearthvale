...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl643-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL64.3 — Alchemist tutor claim after land brew still green.
 * Choice: land alchemy_bench `brew_herbal_tonic` → City claim (assert-only).
 */
describe("CityLands CL64.3 Alchemist tutor claim after land brew", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl643_${Date.now().toString(36)}`,
      "password123",
    );
