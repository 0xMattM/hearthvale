...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl261-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
describe("CityLands CL26.1 builder XP gate on expensive station", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl261_${Date.now().toString(36)}`,
      "password123",
    );
  ...
  it("places forge after enough builder XP (happy)", () => {
    expect(PLAYER_LAND_STATIONS.forge.minBuilderXp).toBe(BUILDER_PLACE_XP);
    ...
    expect(
      getPlayerState(userId)!.buildings.some((b) => b.type === "forge"),
    ).toBe(true);
    // Place still grants builder XP.
    expect(getPlayerState(userId)!.builderXp).toBe(BUILDER_PLACE_XP * 2);
  });

  it("keeps cheaper stations ungated at 0 builder XP (edge)", () => {
    const home = getPlayerState(userId)!;
    db.update(players)
  ...
  it("refuses forge under builder XP gate (failure)", () => {
    const home = getPlayerState(userId)!;
    db.update(players)
    ...
    addItem(home.playerId, "iron_bar", 4);
    addItem(home.playerId, "wood", 4);
    const result = placeLandStation(userId, "forge", boardPos(home));
