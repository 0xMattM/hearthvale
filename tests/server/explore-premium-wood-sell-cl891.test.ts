...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl891-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/** Premium gather mat — Explore book pays more than City (CL10.2 / CL77.1 / CL89.1). */
...
/**
 * CL89.1 — Explore premium wood sell still green.
 * Choice: assert-only Explore > City wood rate (parity with CL77.1; no price retune).
 */
describe("CityLands CL89.1 Explore premium wood sell still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl891_${Date.now().toString(36)}`,
      "password123",
    );
