...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl551-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL55.1 — Explore game_trail leather → City Animal Hunter tutor claim.
 * Choice: assert-only claim e2e (objective still hold_leather) over new tutor ids.
 */
describe("CityLands CL55.1 Explore trail → Animal Hunter tutor claim", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl551_${Date.now().toString(36)}`,
      "password123",
    );
