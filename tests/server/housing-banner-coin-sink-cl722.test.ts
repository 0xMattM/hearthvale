...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl722-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL72.2 — housing banner coin sink still green: land pad place + broke refuse.
 * Choice: assert-only banner sink (broke refuse) over inventing new SKUs.
 */
describe("CityLands CL72.2 housing banner coin sink still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl722_${Date.now().toString(36)}`,
      "password123",
    );
  ...
  it("places banner on land decor_pad as coin sink (happy)", () => {
    ...
    // Reason: L5 soft-unlocks decor_pad; banner is second catalog coin sink @18c.
    db.update(players)
      .set({
        softCurrency: 100,
  ...
  it("keeps banner above planter coin cost with no combat stats (edge)", () => {
    expect(HOUSING_DECOR.banner.coinCost).toBe(18);
  ...
  it("refuses housing banner when broke (failure)", () => {
    const fresh = registerUser(
      `cl722b_${Date.now().toString(36)}`,
      "password123",
    );
