...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl751-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL75.1 — land flour → City vendor sell still green; empty refuse.
 * Choice: assert-only vendor sink (rate already Content Lock) over mill-chain replay.
 */
describe("CityLands CL75.1 land flour City vendor sink still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl751_${Date.now().toString(36)}`,
      "password123",
    );
