...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl851-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL85.1 — Visit presence + nearby trade still green.
 * Choice: assert-only visit host presence + trade ping (parity with CL73.2; no social invent).
 */
describe("CityLands CL85.1 visit presence + nearby trade still green", () => {
  let aliceId = "";
  let aliceName = "";
  let bobId = "";
  let bobName = "";
  ...
  beforeAll(() => {
    ...
    aliceName = `cl851a_${stamp}`;
    bobName = `cl851b_${stamp}`;
