...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl363-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
describe("CityLands CL36.3 builder XP gate on alchemy_bench", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl363_${Date.now().toString(36)}`,
      "password123",
    );
  ...
  it("places alchemy_bench after enough builder XP (happy)", () => {
    expect(PLAYER_LAND_STATIONS.alchemy_bench.minBuilderXp).toBe(
      BUILDER_PLACE_XP,
    );
    ...
    expect(
      getPlayerState(userId)!.buildings.some((b) => b.type === "alchemy_bench"),
    ).toBe(true);
  ...
  it("refuses alchemy_bench under builder XP gate (failure)", () => {
    const home = getPlayerState(userId)!;
    db.update(players)
    ...
    addItem(home.playerId, "wood", 6);
    addItem(home.playerId, "iron_ore", 2);
    const result = placeLandStation(userId, "alchemy_bench", boardPos(home));
