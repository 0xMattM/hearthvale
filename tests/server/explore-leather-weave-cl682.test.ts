...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl682-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL68.2 — Explore leather → land loom weave still green.
 * Choice: assert-only dual-trail hunt → weave_cloth (no Content Lock retune).
 */
describe("CityLands CL68.2 Explore leather → land loom weave still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl682_${Date.now().toString(36)}`,
      "password123",
    );
  ...
  it("hunts Explore leather then weaves cloth on land loom (happy)", () => {
