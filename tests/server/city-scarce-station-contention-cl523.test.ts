...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl523-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
describe("CityLands CL52.3 city scarce station contention assert", () => {
  let aliceId = "";
  let aliceName = "";
  let bobId = "";
  let bobName = "";
  ...
  beforeAll(() => {
    ...
    aliceName = `cl523a_${stamp}`;
    bobName = `cl523b_${stamp}`;
  ...
  it("peer presence does not block city craft start (wait/collect override)", () => {
    ...
    // Alice occupies the scarce kitchen.
    reportPresence({
      userId: aliceId,
      username: aliceName,
      landId: cityLandId,
    ...
    const blocked = craftRecipe(bobId, "cook_fish", pos);
    expect(blocked.ok).toBe(true);
  });

  it("alone at city kitchen crafts OK; land unlimited ignores peer presence (edge)", () => {
    const city = getPlayerState(aliceId)!;
    const kitchen = city.buildings.find((b) => b.type === "kitchen")!;
    const pos = buildingPos(kitchen);
    ...
    // Alone — no other presence near station.
    expect(
      isCityStationContendedByOther(
        "city",
        cityLandId,
    ...
    // Player land: peer standing on same cell must NOT block (unlimited stations).
    ...
    expect(placeLandStation(aliceId, "kitchen", atBoard).ok).toBe(true);
