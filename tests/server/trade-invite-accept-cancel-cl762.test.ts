...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl762-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL76.2 — Trade invite accept + cancel still green.
 * Choice: assert-only nearby accept/cancel + far/own refuse (parity with CL52.1; no trade invent).
 */
describe("CityLands CL76.2 trade invite accept + cancel still green", () => {
  let aliceId = "";
  let aliceName = "";
  let bobId = "";
  let bobName = "";
  ...
  beforeAll(() => {
    ...
    aliceName = `cl762a_${stamp}`;
    bobName = `cl762b_${stamp}`;
