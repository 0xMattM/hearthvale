...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl801-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL80.1 — Farmer tutor claim after land plant/harvest still green.
 * Choice: assert-only claim fidelity (parity with CL64.1; no Content Lock retune).
 */
describe("CityLands CL80.1 Farmer tutor claim after land plant/harvest still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl801_${Date.now().toString(36)}`,
      "password123",
    );
