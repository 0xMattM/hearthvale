...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl711-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL71.1 — Explore wood → land saw_planks still green.
 * Choice: assert-only Explore stump chop → land workshop saw (no Content Lock retune).
 */
describe("CityLands CL71.1 Explore wood → land saw_planks still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl711_${Date.now().toString(36)}`,
      "password123",
    );
