...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl611-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL61.1 — Market buy from other player listing smoke.
 * Choice: assert-only cross-account buy + own-listing refuse (vs CL45.2 cancel-not-yours).
 */
describe("CityLands CL61.1 market buy from other player listing smoke", () => {
  let sellerId = "";
  let buyerId = "";
  ...
  beforeAll(() => {
    ...
    const s = registerUser(`cl611s_${stamp}`, "password123");
    const b = registerUser(`cl611b_${stamp}`, "password123");
