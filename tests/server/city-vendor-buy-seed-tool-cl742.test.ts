...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl742-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL74.2 — City vendor buy seed/tool still green.
 * Choice: assert-only wheat_seed / wooden_hoe buy + broke refuse (parity with CL61.2).
 */
describe("CityLands CL74.2 city vendor buy seed/tool still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl742_${Date.now().toString(36)}`,
      "password123",
    );
