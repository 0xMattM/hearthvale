...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl871-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL87.1 — City scarce kitchen/workshop craft contention still green.
 * Choice: assert-only soft presence lock (parity with CL79.1 / CL52.3 / CL60.3; no qty caps).
 */
describe("CityLands CL87.1 city scarce kitchen/workshop craft contention still green", () => {
  let aliceId = "";
  let aliceName = "";
  let bobId = "";
  let bobName = "";
  ...
  beforeAll(() => {
    ...
    aliceName = `cl871a_${stamp}`;
    bobName = `cl871b_${stamp}`;
