...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl692-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL69.2 — four-map free travel circuit still green after CL67–CL69 economy/social work.
 * Choice: assert-only forward circuit (fare-free holds) over changing travel rules.
 */
describe("CityLands CL69.2 four-map free travel circuit still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl692_${Date.now().toString(36)}`,
      "password123",
    );
