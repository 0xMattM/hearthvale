...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl713-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL71.3 — Explore thicket → Monster Hunter claim still green.
 * Choice: assert-only thicket hunt → City claim (parity with CL53.1; no retune).
 */
describe("CityLands CL71.3 Explore thicket → Monster Hunter claim still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl713_${Date.now().toString(36)}`,
      "password123",
    );
  ...
  it("trail leather does not ready Monster Hunter (edge)", () => {
    const fresh = registerUser(
      `cl713e_${Date.now().toString(36)}`,
      "password123",
    );
