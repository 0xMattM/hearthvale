...
const dbFile = path.join(
  os.tmpdir(),
  `game-cl772-${Date.now()}-${Math.random().toString(16).slice(2)}.db`,
);
...
/**
 * CL77.2 — recipe XP gate fidelity still green for pack_travel_ration / forge_iron_hammer.
 * Choice: assert-only existing gates (cook 25 / blacksmith 20; parity with CL53.3; no retune).
 */
describe("CityLands CL77.2 recipe XP gate fidelity still green", () => {
  let userId = "";

  beforeAll(() => {
    migrateSqlite();
    const reg = registerUser(
      `cl772_${Date.now().toString(36)}`,
      "password123",
    );
